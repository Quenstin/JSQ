...
import com.example.calculator.manager.Parser;
...
public class FormulaView extends LinearLayout {
    ...
    public void setCanClickView(ViewGroup view, boolean isFirst, boolean isRoot) {
        ...
        if (isFirst) {
            view.setBackgroundColor(getResources().getColor(R.color.textBg));
    ...
    private OnClickListener clickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (rootView == null) {
                HdyjLogKt.hdyjLoge("此布局没有root view");
                return;
            }

            isRootSelected = id == rootView.getId();

            //更新选中元素
            for (Map.Entry<String, ViewGroup> view : clickViews.entrySet()) {
                if (view.getValue().getId() == id) {
                    view.getValue().setBackgroundColor(getResources().getColor(R.color.textBg));
    ...
    public boolean hasChildCount(String clickViewName) {
        if (!clickViews.containsKey(clickViewName)) {
            HdyjLogKt.hdyjLoge("在保存的数组中，没有找到对应的控件：" + clickViewName);
            return false;
        }
    ...
    public void removeChildView(String clickViewName, int index) {
        ...
        if (viewGroup == null || viewGroup.getChildCount() <= 1) {
            HdyjLogKt.hdyjLoge("该元素只有一个子节点了，不能操作删除操作");
            return;
        }
        ...
        if (childView instanceof FormulaView) {
            ...
            HdyjLogKt.hdyjLoge("不认识的子节点类型：" + childView.getClass().getSimpleName());
        }
    }
    ...
    public int getChildViewIndex(String clickViewName, String childViewName) {
        if (!clickViews.containsKey(clickViewName)) {
            HdyjLogKt.hdyjLoge("在保存的数组中，没有找到对应的控件：" + clickViewName + "为了保证不崩溃，则删除第一个数据");
            return 0;
        }
        ...
        for (int index = 1; index < clickViewGroup.getChildCount(); ++index) {
            ...
            if (view instanceof SimpleSymbolView) {
                ...
                HdyjLogKt.hdyjLoge("存在不能识别元素：" + view.toString());
            }
        }
    ...
    protected void parseLatexString(ViewGroup viewGroup, String string) {
        HdyjLogKt.hdyjLoge(latexView + ">>>" + getResources().getResourceEntryName(viewGroup.getId()) + " latex：" + string);
        viewGroup.performClick();
