...
public class RichTextManager {
    ...
    public static void bind(String tag, RichText richText){
        ...
        if (richTextMap.containsKey(richText.getId())){
            HdyjLogKt.hdyjLoge("bind--->该" + richText.getId() + "对应的RichText已存在集合，先释放资源并移除，再绑定新的");
    ...
    public static void clear(String tag){
        HdyjLogKt.hdyjLoge("clear--->清除tag绑定的所有RichText，并释放所有的图片缓存，tag：" + tag);
        HashMap<String, RichText> richTextMap = instances.get(tag);
        if (null != richTextMap){
            ...
            HdyjLogKt.hdyjLoge("clear--->该tag绑定的RichText集合为空,tag："  + tag);
        }
    ...
    public static void release(){
        HdyjLogKt.hdyjLoge("release--->释放所有tag绑定的RichText资源");
