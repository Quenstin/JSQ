...
import com.example.calculator.widget.EditView;
...
public class ViewAssembleManager {
    ...
    public int getSelectedViewLevel() {
        if (selectedStruct == null) {
            HdyjLogKt.hdyjLoge("selected struct为null，为了保证程序不崩溃，则添加到根节点");
            return 1;
        }
        ...
        if (pLatexViewName != null && !pLatexViewName.isEmpty()
            ...
            if (formulaViews.containsKey(selectedStruct.getSelectedLatexView())) {
                ...
                HdyjLogKt.hdyjLoge("没找到对应的公式：" + pLatexViewName + "， 为保证程序不崩溃，添加到根节点");
                return 1;
            }
        } else {
    ...
    public void addLine(String latexView, String clickView, int index, int level) {
        ...
        if (latexView == null) {
            ...
            if (formulaView != null) {
                ...
                HdyjLogKt.hdyjLoge("不能找到对应的公式：" + latexView);
            }
        }
    }
    ...
    public void addLine(String pLatexView, String pClickView, String clickView, int level, boolean isFront) {
        ...
        if (pLatexView == null) {
            ...
            if (pFormulaView != null) {
                ...
                HdyjLogKt.hdyjLoge( "不能找到对应的公式：" + pLatexView);
            }
        }
    }
    ...
    /**
     * 根据左右控制键对光标进行移动
     * flag:接收string值 left or right
     */
    public void setLinViewIndex(String flag) {
        int index = getSelectedStruct().getIndex();
        ViewGroup viewGroup = getRootView();
        int gCount = viewGroup.getChildCount() - 1;

        if (flag.equals(getContext().getString(R.string.keyboard_left))) {
            if (index < 0) {
                indexReset(0, viewGroup);
                return;
            }
            if (index == 0) {
                return;
     ...
     * 移除和增加光标
     * @param index 当前插入的位置
     * @param viewGroup 需要插入的布局
     *  还需要优化进行区分
     */
    private void indexReset(int index, ViewGroup viewGroup) {
        deleteLine();
        selectedStruct = new SelectedStruct();
        LineView subViewGroup = new LineView(getContext(), 1);
        formulaViews.put(subViewGroup.getLatexView(), subViewGroup);
        selectedStruct.setIndex(index);
        viewGroup.addView(subViewGroup, index);
    }
        ...
        if (formulaViews.containsKey(LineView.class.getSimpleName())) {
            ...
            if (formulaView.getParentFormulaViewName() != null) {
                ...
                if (parentFormulaView != null) {
                    ...
                    HdyjLogKt.hdyjLoge( "没有找到相应的公式：" + formulaView.getParentFormulaViewName());
                }
            } else {
                if (getRootView() != null) {
    ...
    public void addMathFormula(FormulaView subViewGroup, SelectedStruct selectedStruct) {
        if (subViewGroup == null) {
            HdyjLogKt.hdyjLoge("subViewGroup 为 null");
            return;
        }

        if (selectedStruct == null) {
            HdyjLogKt.hdyjLoge("selected struct为null，请检查代码");
            return;
        }
        ...
        if (sLatexViewName != null && !sLatexViewName.isEmpty()
            ...
            if (formulaViews.containsKey(sLatexViewName)) {
                    ...
                    if (isRootSelected) {
                        ...
                        if (pLatexName == null) {
                            ...
                            if (pFormulaView != null) {
                                ...
                                HdyjLogKt.hdyjLoge( "有选中数据，在整体布局中没有，请查找代码问题");
                            }
                        }
                    } else {
                ...
                HdyjLogKt.hdyjLoge("没有找到对应公式：" + sLatexViewName);
            }
        } else {
            //在根节点的光标处，添加公式
            if (sIndex != -1) {
                ...
                HdyjLogKt.hdyjLoge("在没有选中元素的情况下，必然会有光标，请检查代码");
            }
        }
    ...
    public void addSimpleSymbol(SimpleSymbolView symbolView) {
        if (symbolView == null) {
            HdyjLogKt.hdyjLoge("symbolView 为 null");
            return;
        }

        if (selectedStruct == null) {
            HdyjLogKt.hdyjLoge( "select struct为null，请检查代码，查明原因");
            return;
        }
            ...
            if (sIndex != -1) {
                ...
                if (viewGroup != null) {
                    ...
                    if (childView instanceof FormulaView) {
                        ...
                        HdyjLogKt.hdyjLoge("不能解析的公式：" + childView.getClass().getSimpleName());
                    }
                }
            } else {
                HdyjLogKt.hdyjLoge("在没有选中元素的情况下，必然会有光标，请检查代码");
    ...
    public void notifyClicked(String latexView, String clickViewName, boolean isRootSelected) {
        HdyjLogKt.hdyjLoge( "选中了：" + latexView + "..." + clickViewName);
    ...
    public ConvertResult toLatexString() {
        ...
        if (convertResult.message.isEmpty()) {
            ...
            HdyjLogKt.hdyjLoge( "latex公式为:" + convertResult.message);
